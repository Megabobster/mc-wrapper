...
#		fi
		...
		if echo "$last" | grep -q "^\[..:..:..\] \[Server thread/INFO\]: \[$ARMORSTAND\] $WRAPPER_HALT" ; then
			running="0"
		else
			mc say "$ERR_UNEXPECTED_GRAB_INPUT" "$grab".
		fi
...
# ["$player"] "$message"
trigger_say_command() {
	if [ "$player" = "Server" -a "$message" = "Server backing up." ] ; then
		mc_ignore_armorstand say This may cause some lag.
		mc save-off
		mc save-all flush
	fi
}
...
# <the magic>
	...
	if echo "$line" | grep -q '^\[..:..:..\] \[Server thread/INFO\]: \[.*: Set score of .* for player .* to .*]$' ; then
		...
		while [ "$running" = "2" ] ; do
			read line
			echo "$line"
			if echo "$line" | grep -q '^\[..:..:..\] \[Server thread/INFO\]: Saving worlds$' ; then
				running="0"
				rm mc_input
				rm mc_output
			fi
